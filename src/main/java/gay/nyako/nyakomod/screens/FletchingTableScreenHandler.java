...
public class FletchingTableScreenHandler extends ScreenHandler {
    ...
    public void onContentChanged(Inventory inventory) {
        this.context.run((world, pos) -> {
            ...
            Optional<RecipeEntry<FletchingRecipe>> match = world.getRecipeManager().getFirstMatch(NyakoRecipeTypes.FLETCHING, this.inventory, world);

            ItemStack outputStack = ItemStack.EMPTY;

            if (match.isPresent())
            {
                RecipeEntry<FletchingRecipe> recipe = match.get();
                outputStack = recipe.value().craft(this.inventory, world.getRegistryManager());
            }
    ...
    public ItemStack quickMove(PlayerEntity player, int slotIndex) {
        ...
        if (slot.hasStack()) {
            ...
            if (slotIndex == RESULT_SLOT_INDEX) {
                ...
                slotStack.getItem().onCraftByPlayer(slotStack, player.getWorld(), player);
