...
public class BobmEntity extends BombEntity {
    ...
    protected void explode(World world) {
        float power = 8F;
        Explosion explosion = world.createExplosion(this, this.getDamageSources().explosion(this, getOwner()), null, this.getPos(), power, true, World.ExplosionSourceType.TNT);
        ...
        for (int v = 0; v < list.size(); v++) {
            ...
            if (entity instanceof ServerPlayerEntity serverPlayerEntity) {
                if (!serverPlayerEntity.isImmuneToExplosion(explosion)) {
