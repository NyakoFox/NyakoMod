...
public class CunkShop {
    ...
    public static void loadShopModelFromJson(JsonObject shopJson, ShopData shopData) {
        shopData.name = Text.Serialization.fromJsonTree(shopJson.get("name"));
        shopJson.getAsJsonArray("entries").forEach(entry -> {
            ...
            shopData.add(
                    new ShopEntry(
                            ...
                            Text.Serialization.fromJsonTree(entryJson.get("name")),
                            Text.Serialization.fromJsonTree(entryJson.get("description")),
